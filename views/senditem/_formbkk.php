...
<div class="senditem-_form">
    ...
    <?php $form = ActiveForm::begin(); ?>
        ...
        <?=$form->field($model, 'id_unit_sender')->textInput(['class'=>'setlist','readonly'=>'readonly']);  ?>
        </div><div class="col-md-6">
        <?= $form->field($model, 'name_receiver') ?>
        </div><div class="col-md-6">
        <?= $form->field($model, 'id_reg_type')->textInput(['class'=>'setreglist','readonly'=>'readonly']); ?>
        </div>
...
<?php 
    $js_bkk = "setTimeout(function(){
            controlreg.setValue(8);
           controlunit1.setValue(1);
           $('.qty').val(1);
            controlunit1.lock();
            controlreg.lock();
             },500) ";
